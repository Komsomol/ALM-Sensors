...
async function runExample() {
  ...
  const handleDigitalStateChange = debounce((state, port) => {
    const adjustedState = !state; // Invert the state for digital sensors
  ...
  const handleAnalogVoltageChange = (voltage, port) => {
    const state = voltage > voltageThreshold ? true : false;
    if (state !== prevAnalogState[port]) {
      console.log(`Analog Sensor ${port}: ${state ? 'true' : 'false'} (${voltage}V)`);
